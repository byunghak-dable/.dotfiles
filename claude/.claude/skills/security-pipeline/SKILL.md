...
description: 보안 파이프라인 - CWE Top 25 + STRIDE + OWASP Top 10 통합 검증. security-reviewer 에이전트 호출 포함.
argument-hint: "[파일 경로 또는 --full] [--auto] - 생략 시 변경된 파일 대상"
version: 3.0.0
---
...
수동 호출 시 security-reviewer 에이전트를 Agent tool (subagent_type: general-purpose)로 호출:
...
OWASP Top 10 + CWE Top 25 전체 카테고리를 평가하세요.
